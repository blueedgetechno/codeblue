...
export default class Problems extends React.PureComponent {
  constructor(props){
    ...
    this.state = {
      ...
      isLoggedIn: props.contest.isLoggedIn,
      restartenv: 0,
      mystanding: -1,
      probs: [],
      noftests: [],
  ...
  componentWillReceiveProps(nextProps) {
    this.setState({
      isLoggedIn: nextProps.contest.isLoggedIn
    });
  }
  ...
  runtestpy (i){
    ...
    var res = {n: i-1, error: false, verdict: "none", icon: "hourglass"}
    ...
    exec(cmd,{
      ...
      maxBuffer: 1024*32,
        ...
        if(error !== null){
          res.error = true
          if(atom.config.get("codeblue.beautifyboolean")){
  ...
  submitsolution(){
    ...
    var proburl = "https://codeforces.com/contest/"+ this.state.contestId +"/problem/" + prob.index
  ...
  scrapetests(html, index){
    ...
    this.setState({
      ...
      allverdicts: allverdicts},()=>{
        if(this.state.createnv || this.state.restartenv){
          this.createnv()
        }
      })
  ...
  loadsamplecases(i){
    var url = "https://codeforces.com/contest/"+this.state.contestId+"/problem/"+this.state.probs[i].index
  ...
  scrape(html){
    ...
    for (var i = 2; i < probs.length+2; i++) {
  ...
  loadmystanding(){
    ...
    var url = "https://codeforces.com/api/contest.standings?contestId="+ this.state.contestId +"&from=1&handles=" + atom.config.get("codeblue.codeforcesHandle")
    fetch(url)
      .then(res=> res.json())
      .then(res=> {
        if(res.result.rows[0]){
          this.setState({mystanding: res.result.rows[0].rank})
        }
      })
      .catch(err =>{
        console.log(err)
  ...
  loadsubmissions(){
    ...
    var url = "https://codeforces.com/contest/"+this.state.contestId
    this.fetch(url).then((html) => {
      this.scrape(html);
    }).catch((error) => {
  ...
  fetchactions(){
    ...
    var url = "https://codeforces.com/api/contest.status?contestId="+ this.state.contestId +"&handle="+ atom.config.get("codeblue.codeforcesHandle")
    fetch(url)
      .then(res=> res.json())
  ...
  restart(){
    this.setState({restartenv: 1})
    var url = "https://codeforces.com/api/contest.standings?contestId="+this.state.contestId+"&from=1&count=1"
    fetch(url)
    .then(res=>res.json())
  ...
  componentWillMount(){
    ...
    if(this.props.contest.finished==0 && intervaltime>0){
      setInterval(()=>{this.loadsubmissions()},intervaltime*1000)
    }

    if(this.props.contest.openinbrowser){
      const { exec } = require('child_process');
      var cmd = "start "+"https://codeforces.com/contest/"+this.state.contestId+"/problems"
      exec(cmd,(error, stdout, stderr)=>{
        if(error==null){
  ...
  hide(){
    var tohide = document.getElementById('standingview')
  ...
  render(){
    return (
      <div className="display">
        <button className="refreshproblems" title="refresh problems" onClick={this.restart.bind(this)}>Restart</button>
        ...
        {this.state.probs.length ?
          ...
          tempvalue={this.state.temp}
          probs={this.state.probs}
          refreshbutton={this.loadsubmissions.bind(this)}
          />
        : null}
        ...
        {this.state.probs.length ? <Actions
          ...
          LoginCodeforces={this.props.LoginCodeforces}
         /> : null }
